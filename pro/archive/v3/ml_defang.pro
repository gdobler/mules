...
;  print, cellnum, total((sqrt(alphar^2 + alphat^2))[9:19]), form='(I5,D6.2)'
...
;;        delxi = cells[cellnum].xcell - stars.xstar[stind]
;;        delyi = cells[cellnum].ycell - stars.ystar[stind]
;;        ri    = sqrt(delxi^2+delyi^2)
;;        ti    = atan(delyi, delxi)
        mi    = stars.rein[stind]
;;        denom = rcell^2 + ri^2 - 2*rcell*ri*cos(tcell-ti)
;;
;;        talphar = total(mi*mi*(rcell - ri*cos(tcell-ti))/denom)
;;        talphat = total(mi*mi*ri*sin(tcell-ti)/denom)
;;
